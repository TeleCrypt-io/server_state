...
def main() -> int:
    ...
    if (
        ...
        or args.timeout <= 0
        or (args.combined_limit is not None and args.combined_limit <= 0)
    ...
    try:
        ...
        while selector.get_map():
            ...
            for key, _ in selector.select(wait_for):
                ...
                if args.combined_limit is not None:
                    remaining = min(remaining, args.combined_limit - combined_used)
                if remaining > 0:
            ...
            if process.poll() is not None and selector.get_map() and close_at is None:
                # A command that leaves descendants holding inherited descriptors must not
                # keep this wrapper alive. Descendants are in the command's own process group.
                terminate_group()
    ...
    if overflow:
        print("bounded command output exceeded its limit", file=sys.stderr)
        return 1
    if timed_out:
